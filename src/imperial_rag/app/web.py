...
SOURCE_DRAWER_SELECTED_KEY_PREFIX = "source_drawer_selected"
...
def main() -> None:
    ...
    st.title(APP_TITLE)
    _render_source_drawer_styles(st)
...
def _render_retrieved_files(st: Any, groups: list[RetrievedFileGroup], message_index: int) -> None:
    open_state_key = f"source_drawer_open_{message_index}"
    with st.container(border=True, key=f"source-summary-{message_index}"):
        info_col, action_col = st.columns([4, 2], vertical_alignment="center")
        with info_col:
            st.markdown(f":material/description: **{_source_count_label(len(groups))}**")
            st.caption("Документы, использованные в ответе")
        with action_col:
            st.button(
                "Открыть источники",
                key=f"source-drawer-open-{message_index}",
                icon=":material/quick_reference_all:",
                width="stretch",
                on_click=_set_session_state_value,
                args=(st, open_state_key, True),
            )
    if st.session_state.get(open_state_key, False):
        _render_source_drawer(st, groups, message_index, open_state_key)


def _render_source_drawer(
    st: Any,
    groups: list[RetrievedFileGroup],
    message_index: int,
    open_state_key: str,
) -> None:
    selected_state_key = f"{SOURCE_DRAWER_SELECTED_KEY_PREFIX}_{message_index}"
    available_keys = {group.file_key for group in groups}
    if st.session_state.get(selected_state_key) not in available_keys:
        st.session_state[selected_state_key] = groups[0].file_key

    def close_source_drawer() -> None:
    ...
    @st.dialog(
        ...
        width="large",
        icon=":material/quick_reference_all:",
        on_dismiss=close_source_drawer,
    )
    def source_drawer() -> None:
        query = st.text_input(
            "Поиск по файлам",
            key=f"source-drawer-search-{message_index}",
            placeholder="Поиск по файлам",
            label_visibility="collapsed",
            icon=":material/search:",
        )
        filtered_groups = _filter_retrieved_file_groups(groups, query or "")
        if not filtered_groups:
            st.caption("Файлы не найдены.")
            return
        if st.session_state.get(selected_state_key) not in {group.file_key for group in filtered_groups}:
            st.session_state[selected_state_key] = filtered_groups[0].file_key

        for group_index, group in enumerate(filtered_groups):
            selected = group.file_key == st.session_state.get(selected_state_key)
            with st.container(border=True, key=f"source-drawer-row-{message_index}-{group_index}"):
                select_col, download_col = st.columns([4, 1.5], vertical_alignment="center")
                with select_col:
                    st.button(
                        group.file_name,
                        key=f"source-drawer-select-{message_index}-{group_index}",
                        icon=":material/check_circle:" if selected else ":material/description:",
                        type="secondary" if selected else "tertiary",
                        width="stretch",
                        on_click=_set_session_state_value,
                        args=(st, selected_state_key, group.file_key),
                    )
                with download_col:
                    ...
                    st.download_button(
                        ...
                        key=f"source-drawer-download-{message_index}-{group_index}",
                        help=f"Скачать {group.file_name}",
                        disabled=disabled,
                        width="stretch",
        ...
        selected_group = _selected_retrieved_file_group(
            filtered_groups,
            str(st.session_state.get(selected_state_key) or ""),
        )
        if selected_group is None:
            return
        st.subheader("Предпросмотр")
        st.markdown(f"**{selected_group.file_name}**")
        st.caption(f"Путь: {selected_group.display_path}")
        st.text(selected_group.preview_text)
        st.caption("Предпросмотр показывает фрагменты документа. Полный файл доступен для скачивания.")
...
def _filter_retrieved_file_groups(
    groups: list[RetrievedFileGroup],
    query: str,
) -> list[RetrievedFileGroup]:
    ...
    return [
        group
        for group in groups
        if normalized_query in f"{group.file_name}\n{group.display_path}".casefold()
    ]


def _selected_retrieved_file_group(
    groups: list[RetrievedFileGroup],
    selected_file_key: str,
) -> RetrievedFileGroup | None:
    return next((group for group in groups if group.file_key == selected_file_key), groups[0] if groups else None)


def _set_session_state_value(st: Any, key: str, value: Any) -> None:
    st.session_state[key] = value


def _render_source_drawer_styles(st: Any) -> None:
